...
- `summary_zh` 和 `summary_ja` 只写摘要，不要写太长。
- `content_zh` 和 `content_ja` 才是正文区域。
...
### 6.1 `highlights_zh` / `highlights_ja` 填写示例

```json
["连铸设备", "连轧设备", "冷轧设备", "表面强化"]
```

### 6.2 `stats_zh` / `stats_ja` 填写示例
