...
3. For local development, create a project-root `.env` from `.env.example`.
4. Set these variables in the shell or deployment environment:
   - `PUBLIC_POCKETBASE_URL`
   - `PB_ADMIN_EMAIL`
   - `PB_ADMIN_PASSWORD`
5. Run `npm run cms:setup` to create or update the required collections
6. Run `npm run cms:check` to verify admin auth and collection visibility
7. Run `npm run cms:seed` for initial import
8. Run `npm run cms:seed:reset` only when you explicitly want to wipe and re-import seeded collections
...
2. `npm install`
3. Ensure `PUBLIC_POCKETBASE_URL`, `PB_ADMIN_EMAIL`, and `PB_ADMIN_PASSWORD` are set in the
   production shell, PM2, or Baota environment
4. `npm run cms:setup`
5. `npm run cms:check`
6. `npm run build`
...
- The `cms:*` scripts still load `PUBLIC_POCKETBASE_URL` from a local `.env` file when needed.
- In production-like environments, `PB_ADMIN_EMAIL` and `PB_ADMIN_PASSWORD` no longer load from
  project-root `.env` by default.
- Use `CMS_ALLOW_LOCAL_ADMIN_ENV=true` only as an explicit emergency override when you
  intentionally want file-based CMS admin credentials in a production-like environment.
